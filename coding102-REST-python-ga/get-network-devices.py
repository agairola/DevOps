...
def getNetworkDevices(ticket):
	...
	for i in r_json["response"]:	
                print('{:15}'.format(i["managementIpAddress"]) + "   " + '{:4}'.format(str(i["locationName"])) + "   " + '{:53}'.format(i["series"]) + "  " + i["reachabilityStatus"])
...
def getHosts(ticket):
	...
	#Iterate through network device data and print the id and series name of each device
	for i in r_json["response"]:
		#print('{:10}'.format(i["hostIp"]) + "   " + '{:10}'.format(str(i["hostType"])) + "   " + '{:53}'.format(i["connectedAPName"]) + "  " + i["vlanId"])
		print('{:15}'.format(i["hostIp"]) + "   " + '{:10}'.format(str(i["hostType"])) + "  " + i["vlanId"])
